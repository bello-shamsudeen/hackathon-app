  ...
  return (
    <PhoneFrame showNav={false}>
      <div style={{ padding: 20 }}>
        <button onClick={() => navigate('/bank/home')} style={{ background: 'none', border: 'none', color: 'var(--bank-ink-dim)', fontSize: 13, cursor: 'pointer', marginBottom: 18, padding: 0 }}>
          &larr; Back
        </button>

        <div style={{ textAlign: 'center', marginBottom: 26 }}>
          <Avatar name={profile.full_name} photoUrl={profile.avatar_data_url} size={84} />
          <div style={{ fontSize: 22, fontWeight: 700, fontFamily: 'var(--font-display)', marginTop: 14 }}>
            {profile.full_name}
          </div>
          <div style={{ fontSize: 12, color: 'var(--bank-ink-dim)' }}>Member since {profile.member_since}</div>
        </div>

        <div style={{ background: '#fff', border: '1px solid var(--bank-border)', borderRadius: 12, overflow: 'hidden', marginBottom: 18 }}>
        ...
        <div style={{
          ...
          <div>
            <div style={{ fontSize: 13, fontWeight: 600 }}>Protected by Aegis</div>
...
function InfoRow({ label, value, mono, last }) {
  return (
    <div style={{
      ...
      borderBottom: last ? 'none' : '1px solid var(--bank-border)', fontSize: 14
    }}>
