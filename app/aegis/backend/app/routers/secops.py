...
@router.get("/metrics")
def live_metrics(minutes: int | None = None):
    """Live catch-rate / false-positive-rate / precision / latency percentiles
    from real decisions. `?minutes=N` scopes to the last N minutes (one fresh
    simulator run) instead of all history."""
    return compute_live_metrics(since_minutes=minutes)
