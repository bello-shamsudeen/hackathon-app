...
export default function SendMoney({ session }) {
  ...
  const handleConfirm = async () => {
    ...
    // Reacts to the REAL Aegis decision — this is what makes the consumer
    // side reflect the actual pipeline, not a static "success" message.
    const scoreRes = await fetch(`/score/session/${session.session_id}`, { method: 'POST' })
    const scoreData = await scoreRes.json()

    setDecision(scoreData)
    setLoading(false)
    setStep(4)
  }
